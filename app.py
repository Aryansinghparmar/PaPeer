...
from backend.config import CHECKPOINT_DB_PATH, OPENAI_CHAT_MODEL, OPENAI_RENAME_MODEL
...
@st.cache_resource
def get_graph():
    return build_graph(db_path=CHECKPOINT_DB_PATH)
...
def load_session_chats(session_id: str) -> list[dict]:
    ...
    try:
        ...
        if not state or not state.values:
            return []
        chats = []
        chats = clean_history_messages(state.values.get("messages", []))
        return chats
    except Exception:
        return []
    ...
    if is_btw:
        ...
        with st.chat_message("assistant"):
            ...
            run_record = record_run(
                ...
                session_id=active_sid,
                state=final_values,
                latency_seconds=monotonic_seconds() - run_started,
                model_names=[OPENAI_CHAT_MODEL],
                usage_by_model=usage_callback.usage_metadata,
            )
            state_snapshot["observability"] = {
                "latency_seconds": run_record["latency_seconds"],
                "estimated_cost_usd": run_record["estimated_cost_usd"],
